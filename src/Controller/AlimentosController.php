...
*/
	public function index()
    {
		...
		$marcas = $this->Marcas->find('list', ['keyField' => 'id','valueField' => 'nombre'])->where(['marcas_tipos_id'=>7])->order(['nombre' => 'ASC']);
		...
		$marcas2 = $this->Marcas->find('all')->where(['marcas_tipos_id'=>7])->order(['nombre'=>'ASC']);
		$marcas2->toArray();
		
		$this->set(compact('marcas2'));

		$this->loadModel('Publications');
		$publications = $this->Publications->find('all')->where(['fecha_hasta >=CURRENT_DATE()','habilitada' =>'1','ubicacion'=>'10'])->order(['id' => 'DESC'])->all();
		$this->set('publications_alimento',$publications->toArray());
		
		$this->request->session()->write('publications_alimento',$publications->toArray());
