...
<div class="col-md-9" >
                    <div class="product-item-3">
						...
						<?php
						echo '<div class=row>'.$this->element('alimento_search').'</div>';
						echo '<div class=row><br>'. $this->element('alimento_marcas');
						echo $this->element('alimento_sin_result'); 
						echo '</div>';
						?>
...
<div class="col-md-3">
    <div class="product-item-5"> 
		<div class="product-content">
			<div class="row">
			<?php echo $this->element('alimento_cart_resum'); ?>
			</div> <!-- /.row -->
		</div> <!-- /.product-content -->
	</div>
	<div class="product-item-5">	
		...
		<div class="row" >   
			<?php echo $this->element('alimento_cart_botton'); ?>
			<div class="cartresul">			
			<?php echo $this->element('alimento_cart_result'); ?>
			</div>
		</div> <!-- /.row -->
		</div> <!-- /.product-content -->
	</div> <!--.product-item-1 -->  
	<?php
/*if(!is_null($this->request->session()->read('promo5'))) {
    $promo5 = $this->request->session()->read('promo5');
if ($promo5['url_metodo']!='') {
if ($promo5['url_campo']!='')
echo $this->Html->image('publicaciones/'.$promo5['imagen'],['url'=>['controller'=>$promo5['url_controlador'],'action'=>$promo5['url_metodo'],$promo5['url_campo']],'alt'=>'Drogueria Sur S.A.','width'=>'100%']);
else echo $this->Html->image('publicaciones/'.$promo5['imagen'],['url'=>['controller'=>$promo5['url_controlador'],'action'=>$promo5['url_metodo']],'alt'=>'Drogueria Sur S.A.','width'=>'100%']);
}
else echo $this->Html->image('publicaciones/'.$promo5['imagen'],['width'=>'100%']);
}
if(!is_null($this->request->session()->read('promo6'))) {
    $promo6 = $this->request->session()->read('promo6');
if ($promo6['url_metodo']!='')
{

if ($promo6['url_campo']!='') echo $this->Html->image('publicaciones/'.$promo6['imagen'],['url'=>['controller'=>$promo6['url_controlador'],'action'=>$promo6['url_metodo'],$promo6['url_campo']],'alt'=>'Drogueria Sur S.A.','width'=>'100%']);
else echo $this->Html->image('publicaciones/'.$promo6['imagen'],['url'=>['controller'=>$promo6['url_controlador'],'action'=>$promo6['url_metodo']],'alt'=>'Drogueria Sur S.A.','width'=>'100%']);
}
else echo $this->Html->image('publicaciones/'.$promo6['imagen'],['width'=>'100%']);  
}*/
?>
...
<?php
foreach ($publications_alimento as $publication): 
