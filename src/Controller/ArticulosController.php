...
class ArticulosController extends AppController
{
	...
	{
		if (in_array($this->request->action, ['index_admin', 'edit_admin', 'delete', 'add', 'nuevos', 'index_imagen', 'imagenesreset', 'imagenreset', 'ordenupdate', 'search_articulos', 'buscar', 'buscarfactura', 'positionArticles', 'validatePositionArticles'])) {
	...
	public function positionArticles()
	{
	...
	public function index_admin()
	{
		...
		$this->paginate = [
			...
			'limit' => 500,
			'maxLimit' => 1000,
			'order' => array('Articulos.fecha_alta' => 'DESC')
		];
		//$this->set('_serialize', ['articulos']);
		$this->set('titulo', 'Articulos');
		$this->categoriaylaboratorio();
	 ...
	 */
	public function validatePositionArticles()
	{
		$this->viewBuilder()->setLayout("vacio");

		try {
			$arrayPositions = $this->request->input('json_decode', true);

			if (!$arrayPositions || !isset($arrayPositions['data']) || empty($arrayPositions['data'])) {
				return $this->response->withType('application/json')
					->withStringBody(json_encode(['error' => 'Datos inválidos o vacíos recibidos']));
			}
			$positions  = [];
			$codigoBarrass = [];
			$arrayFinal = [];

			foreach ($arrayPositions['data'] as $item) {
				$valor = substr_replace($item['posicion'], "", 6, 1);
				$valorfinal = substr_replace($valor, "", 4, 1);
				array_push($positions, $valorfinal);
				array_push($codigoBarrass, $item['codigo_barras']);
			}

			$articulos = $this->Articulos->find('all')->select('clave_amp', 'posicion', 'categoria_id')
				->where(['posicion IN' => $positions])
				->toArray();

			if (!empty($articulos)) {
				foreach ($articulos as $item) {

					if (in_array($item['categoria_id'], [1, 2, 6, 7])) {
						array_push($arrayFinal, ['clave_amp' => $item['clave_amp'], 'nuevaPosicion' => 99990909]);
					} else {
						array_push($arrayFinal, ['clave_amp' => $item['clave_amp'], 'nuevaPosicion' => "09990909"]);
					}
				}
			}


			$articulosBarras = $this->Articulos->find('all')->select(['clave_amp', 'codigo_barras'])
				->where(['codigo_barras IN' => $codigoBarrass, 'eliminado' => 0])
				->indexBy('codigo_barras')
				->toArray();

			if (!empty($articulosBarras)) {
				foreach ($arrayPositions['data'] as $item) {
					if (isset($articulosBarras[$item['codigo_barras']])) {
						$articulo = $articulosBarras[$item['codigo_barras']];
						$arrayFinal[] = [
							'clave_amp' => $articulo->clave_amp,
							'nuevaPosicion' => $item['posicion']
						];
					}
				}
			}

			if (empty($articulos) && empty($articulosBarras)) {
				return $this->response->withType('application/json')
					->withStringBody(json_encode(['message' => 'No se encontraron artículos']));
			}

			return $this->response->withType('application/json')
				->withStringBody(json_encode($arrayFinal));
		} catch (\Exception $e) {
			return $this->response->withType('application/json')
