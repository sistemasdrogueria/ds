...
class ComprobantesController extends AppController
{
    ...
    {
		if (in_array($this->request->action, ['search','index','view_admin','search_admin','index_admin','home','downloadfile','traza','downloadfiletxt','downloadfiletrazatxt','view','downloadfiletxt2','downloadfiletxtselect','excel','trazapdf','lotevctopdf'])) {
	...
	public function search_admin()
    {
		...
		{	
			...
			if ($this->request->data['terminobuscarf']!= null)
				...
				//$this->request->data['terminobuscarf'];
			else
				$termsearchf ="";
	...
	*/
	...
	public function downloadfiletxt ($nota = null,$fecha =null){
		...
		else
		{
			...
			$nombreArchivo= 'F'.str_pad($nota, 6, "0", STR_PAD_LEFT).$codigo;// str_pad($client['codigo'], 6, "0", STR_PAD_LEFT);
			$file = new File('temp'. DS .'Comprobantes'. DS .$nombreArchivo.'.TXT', true, 0777);
			$file->write("\n",'w');
			foreach ($query as $row): 
