...
class AccesoriosYProductosMedicosController extends AppController
{
		...
		public function isAuthorized()
    {
		 ...
		 if (in_array($this->request->action, ['index','delete_admin','index_admin','add_admin','edit_admin','view_admin','consolidado','search'])) {
  ...
  /**
	...
	public function categoriaylaboratorio()
	{
		...
		if ($this->request->session()->read('Laboratorios2')== null)
		{
		...
		$articulosA = $this->Articulos->find('list',['keyField' => 'Laboratorios.id','valueField'=>'Laboratorios.nombre'])
			...
			->where(['Articulos.eliminado'=>0,'Laboratorios.id NOT IN(65,359,375)','Laboratorios.eliminado'=>0,'Articulos.categoria_id in (2,4)'])->group(['Laboratorios.nombre'])->order(['Laboratorios.nombre']);
				
		//$this->request->session()->write('articulosL',$articulosA->toArray());
		...
		$laboratorios2 =$this->request->session()->read('Laboratorios2');
		$this->request->session()->write('Categorias',$categorias->toArray());	
			...
			->where(['Articulos.eliminado'=>0,'Laboratorios.eliminado'=>0,'Laboratorios.id NOT IN(360,3,65,359,375)','Articulos.categoria_id in (2,4)'])->group(['Laboratorios.nombre'])->order(['Laboratorios.nombre']);
		...
		$this->set('laboratoriosAPM',$laboratorios2);
		$this->set('laboratorios',$this->request->session()->read('Laboratorios'));
	...
	public function index()
    {
		...
		$articulosA = $this->Articulos->find('all')//,['keyField' => 'Laboratorios.id','valueField'=>'Laboratorios.nombre'])
		...
		//->hydrate(false)
			...
			->join([
				...
				'type' => 'INNER',
				 'conditions' => [
				'Laboratorios.id = Articulos.laboratorio_id', 'Laboratorios.eliminado=0'
		...
		$this->set('PatagoniaMed',$articulosA->toArray());
	...
	public function search($lab = null)
    {
		if ($this->request->is('post'))
		{	
			if ($this->request->data['laboratorio_id']!= null)
			...
			if ($this->request->data['terminobuscar']!= null)
			{
				...
				$termsearch ="";
			$this->request->session()->write('termsearchapm',$termsearch);
			$this->request->session()->write('laboratorioidapm',$laboratorioid);
		}
		else 
		{
			if ($lab !=null)
			$this->request->session()->write('laboratorioidapm',$lab);
		    $laboratorioid = $this->request->session()->read('laboratorioidapm');
			$termsearch = $this->request->session()->read('termsearchapm');
		}
		...
		if ($laboratorioid !=0)
		if (($laboratorioid==382) || ($laboratorioid==462)|| ($laboratorioid==500) ||($laboratorioid==503)||($laboratorioid==457))
						$articulosA->andWhere(['Articulos.laboratorio_id in (382,462,500,503,457)']);
					else
					if (($laboratorioid==477) || ($laboratorioid==468))
						$articulosA->andWhere(['Articulos.laboratorio_id in (477,468)']);
						else
						if (($laboratorioid==3) || ($laboratorioid==360))
						$articulosA->andWhere(['Articulos.laboratorio_id in (3,360)']);
						else
						if (($laboratorioid==143) || ($laboratorioid==337)||($laboratorioid==159))
						$articulosA->andWhere(['Articulos.laboratorio_id in (143,337,159)']);
						else
						if (($laboratorioid==65) || ($laboratorioid==359) ||  ($laboratorioid==385))
						$articulosA->andWhere(['Articulos.laboratorio_id in (65,359,385)']);
						else
						if (($laboratorioid==595) || ($laboratorioid==598))
						$articulosA->andWhere(['Articulos.laboratorio_id in (595,598)']);
						else
						if (($laboratorioid==368) || ($laboratorioid==600))
						$articulosA->andWhere(['Articulos.laboratorio_id in (368,600)']);
						else
		...
		if ($articulosA!=null)
		{
			...
			$articulosA->andWhere(['Articulos.eliminado=0','Articulos.categoria_id in (2,4)','Articulos.stock<>"D"'])->group(['Articulos.id'])
						->order(['Articulos.descripcion_pag' => 'asc']);
