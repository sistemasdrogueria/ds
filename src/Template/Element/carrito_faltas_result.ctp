...
<tr id="trfalta<?php echo $articulo['id']?>">
...
{
		echo $this->Form->input($encabezado.'cantidad',['id'=>'tab'.$indice,'data-cantidad-car'=>$cantidadencarrito, 'data-id-input'=>'tab'.$indice,'tabindex'=>$indice,'data-id'=> $articulo['id'],'data-pv-id'=>  $articulo['descuentos'][1]['id'],'value'=>$cantidadencarrito, 'onkeypress'=>'return soloNumeros(event)','onchange'=>'eliminarItemsFalta(this);','target'=>'_blank'  , 'autocomplete'=>'off']);

}
else
{
	echo $this->Form->input($encabezado.'cantidad',[ 'id'=>'tab'.$indice,'data-cantidad-car'=>$cantidadencarrito, 'data-id-input'=>'tab'.$indice,'tabindex'=>$indice,'value'=>$cantidadencarrito, 'data-id'=> $articulo['id'],'data-pv-id'=> 0,'class'=>'formcartcantfaltas ', 'onkeypress'=>'return soloNumeros(event)','onchange'=>'eliminarItemsFalta(this);','target'=>'_blank'  , 'autocomplete'=>'off']);
...
{
	echo $this->Form->input($encabezado.'cantidad',[ 'id'=>'tab'.$indice,'data-cantidad-car'=>$cantidadencarrito, 'data-id-input'=>'tab'.$indice,'tabindex'=>$indice,'data-id'=> $articulo['id'],'data-pv-id'=> 0,'value'=>$cantidadencarrito,'class'=>'formcartcantfaltas ', 'onkeypress'=>'return soloNumeros(event)','onchange'=>'eliminarItemsFalta(this);','target'=>'_blank'  , 'autocomplete'=>'off']);
}
}
}
else
...
<td class='colprecio' style=" border: solid #fff; border-width: 0px 1px 0px 1px;">
    <?php echo $this->element('precio_publico',['articulo'=>$articulo ,'descuento_pf'=>$descuento_pf,'coef'=>$coef,'coef_pyf'=>$coef_pyf] );?>
</td>
<td class='colprecio' style=" border: solid #fff; border-width: 0px 1px 0px 1px;">
...
if ($articulo['fv_cerca']==1) { echo $this->Html->image('fv.png',['title' => 'Vencimiento Cercano']);	}
if (($articulo['imagen']!="perfumeria.jpg" && $articulo['imagen']!="sinimagen.png") && ($articulo['imagen']!="medicamento.jpg")) echo $this->Html->image('foto.png',['title' => 'Ampliar la foto','class'=>'imgFoto','data-id'=>$articulo['imagen']]);
...
<td class="actions">
<a href="#" onclick="preguntarSiNoFaltas(<?php echo $articulo['carritos_faltas'][0]['id'] ?>,<?php echo $articulo['carritos_faltas'][0]['articulo_id'] ?>)">
<?php echo $this->Html->image("delete_ico.png", ["alt" => "Quitar de de Faltas"]); ?></a>
</td>
</tr>
<?php endforeach; ?>
</tbody>
