...
<td class="actions">
<?php 
if (($transfersImport->transfers_files_laboratorio['id']!= 9) && ($transfersImport->transfers_files_laboratorio['id']!= 18) 
&& ($transfersImport->transfers_files_laboratorio['id']!= 24))
...
else
{
        echo $this->Html->link(
                'OP 1 ',
                ['controller' => 'TransfersProveedors', 'action' => 'explusive_admin',$transfersImport->id,1,'_full' => true]
            );
            echo $this->Html->link(
                'OP 2 ',
                ['controller' => 'TransfersProveedors', 'action' => 'explusive_admin',$transfersImport->id,2,'_full' => true]
            );
            echo $this->Html->link(
                'OP 5',
                ['controller' => 'TransfersProveedors', 'action' => 'explusive_admin',$transfersImport->id,5,'_full' => true]
            );
        echo 'NO BORRAR';

        if (is_null($transfersImport->txt_generado))
        $imagen_sincr = "admin/icon_generar_txt.png";
        else
        $imagen_sincr = "admin/icon_generar_txt_ok.png";
echo $this->Html->image($imagen_sincr,["alt" => "Edit",'url' =>['controller' => 'TransfersProveedors', 'action' => 'file_admin',  $transfersImport->id]]);
...
if ($transfersImport->estado >0 && $transfersImport->estado!=27 && $transfersImport->estado!=25)
{
        ...
        if ($transfersImport->estado==7 || $transfersImport->estado==11 || $transfersImport->estado==4) 
