 ...
 */
class PatagoniaMedController extends AppController
{
  ...
  /**
	...
	public function index()
    {
		...
		$articulosA = $this->Articulos->find('all')
			...
			->where(['Articulos.eliminado'=>0,'Articulos.categoria_id in (1,6,7)','d.tipo_venta' => "D"])->group(['Articulos.laboratorio_id']);
	...
	public function search($lab = null)
    {
		...
		{
			if ($lab !=null)
			$this->request->session()->write('laboratorioid',$lab);
		    $laboratorioid = $this->request->session()->read('laboratorioid');
			$termsearch = $this->request->session()->read('termsearchpt');
		}
