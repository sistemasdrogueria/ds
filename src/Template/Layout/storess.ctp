...
echo $this->Html->css('templatemo-style-ss.min');echo $this->Html->css('stylebar-ss.min');
echo $this->Html->script('modernizr-2.6.2.min');echo $this->Html->script('jquery.quick.pagination.min');?>
...
<div class=logods>
<?php echo $this->Html->image('logo_hss.png',['alt'=>'Drogueria Sur S.A.','id'=>'logo_web','url'=>['controller'=>'Carritos','action'=>'index']]);?>
</div>
</div>
...
<li style='float: right; height:40px; margin:0 5px 0 5px; '> <?php echo $this->Html->image('icono_carro.png',['url'=>['controller' => 'Carritos', 'action' => 'view']], ['alt' => 'Ver carro','id'=>'opener']);?></li>
<?php //if ($this->request->session()->read('Auth.User.comunidadsur')>0)	
echo '<li style="float: right;  height:40px; margin:0 5px 0 5px;">'.$this->Html->image('logo_comunidad.png', ['alt' => 'Comunidad Sur','url'=>'https://www.comunidadsur.com.ar/']).'</li>';?>


<?php if ($this->request->session()->read('Auth.User.farmapoint')==1) 
echo '<li style="float: right;  height:40px; margin:0 5px 0 5px;">'.$this->Html->image('logo_TFP.png', ['controller' => 'Carritos', 'action' => 'farmapoint']).'</li>';?>
