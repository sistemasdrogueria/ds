...
$objPHPExcel->getProperties()->setTitle("Detalle de Facturas"); //titulo
...
//titulos de columnas
  ...
  foreach ($facturascabeceras as $facturasCabecera):
		...
		*/
		...
		$objPHPExcel->getActiveSheet()->SetCellValue("C$fila", $clientes[$this->request->session()->read('cliente_id')]);		
		...
		foreach ($facturasCabecera['facturas_cuerpos_items'] as $fci): 
			...
			$objPHPExcel->getActiveSheet()->SetCellValue("B$fila",$fci['descripcion']);
			$objPHPExcel->getActiveSheet()->mergeCells("B$fila:D$fila");
			$objPHPExcel->getActiveSheet()->SetCellValue("E$fila", str_pad($fci['codigo_barra'], 13, "0", STR_PAD_LEFT));
			$objPHPExcel->getActiveSheet()->SetCellValue("F$fila", $fci['troquel']);
			/*
			$objPHPExcel->getActiveSheet()->SetCellValue("B$fila",$fci['descripcion']);
			$objPHPExcel->getActiveSheet()->mergeCells("B$fila:D$fila");
			$objPHPExcel->getActiveSheet()->SetCellValue("E$fila", str_pad($fci['codigo_barra'], 13, "0", STR_PAD_LEFT));
...
$namefile= '"Factura_detalle_'.$clientes[$this->request->session()->read('cliente_id')].'.xlsx"';
//// nombre del archivo
