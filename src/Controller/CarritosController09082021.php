		...
		*/
		...
		if (($carrito['categoria_id'] !=5) && ($carrito['categoria_id'] !=4)  && ($carrito['categoria_id'] !=3) &&($carrito['categoria_id'] !=2))
			...
			else
			{
				...
				if ($carrito['articulo']['iva'] ==1)
						...
						$subtotal = $precio*$cant_carrito;		
							/*				
						if (($this->request->session()->read('Auth.User.codigo_postal')==9410 || $this->request->session()->read('Auth.User.codigo_postal')==9420) && ($carrito['articulo']['id']==16144 || $carrito['articulo']['id']==50191 ||$carrito['articulo']['id']==50367||$carrito['articulo']['id']==2672||$carrito['articulo']['id']==25172))
						{
						$subtotal = $subtotal*1.224;
						}*/
		...
		else
		{
			...
			if ($carrito['tipo_oferta_elegida']=='D')
			{
				...
				if (($this->request->session()->read('Auth.User.codigo_postal')==9410 || $this->request->session()->read('Auth.User.codigo_postal')==9420) && ($carrito['articulo']['id']==16144 || $carrito['articulo']['id']==50191 ||$carrito['articulo']['id']==50367||$carrito['articulo']['id']==2672||$carrito['articulo']['id']==25172 ||
				$carrito['articulo']['id']==50367||$carrito['articulo']['id']==2672|| $carrito['articulo']['id']==25172 || $carrito['articulo']['id']==9400||$carrito['articulo']['id']==8907||$carrito['articulo']['id']==4571 || $carrito['articulo']['id']==3000|| $carrito['articulo']['id']==3113 || $carrito['articulo']['id']==5257||$carrito['articulo']['id']==5951||$carrito['articulo']['id']==10459 || $carrito['articulo']['id']==15666||$carrito['articulo']['id']==18478||$carrito['articulo']['id']==19927))
...
/*
	...
	public function index()
    {
		...
		$ofertasX = $this->Ofertas->find('all')->where(['oferta_tipo_id in (2,4,5,6,11,19)','activo'=>1,'fecha_desde <= CURRENT_DATE()','fecha_hasta >=CURRENT_DATE()'])->order(['oferta_tipo_id'=>'ASC','orden' => 'ASC']);
		...
		$publication_con = $this->Publications->find('all')->where(['fecha_hasta >=CURRENT_DATE()','habilitada' =>'1','ubicacion'=>'2','localidad'=>$codigo_postal])->unionAll($publication_sin)->order(['orden' => 'ASC'])->limit(4);
	...
	public function view()
    {
		...
		//$fecha = Time::createFromFormat('d/m/Y',$fecha,'America/Argentina/Buenos_Aires');
		 ...
		 if ($this->request->session()->read('Auth.User.farmapoint'))
		 {
			 ...
			 'Descuentos' => [					
				 'queryBuilder' => function ($q) {
					 return $q->where(['tipo_oferta in ("RV","RR","OR","TD","RL","SS","FR","TH","FP","VC")']); // Full conditions for filtering
				 }
			 ...
			 ->hydrate(false)
				 ->join([
					 ...
					 'd.tipo_venta = "D"',
					 'd.fecha_hasta >=' => $fecha,
					 'd.tipo_oferta in ("RV","RR","OR","TD","RL","SS","FR","TH","FP","VC")'
					 ]		
				 ]
				 )
				 ->join([
		 ...
		 else
		 {
			 ...
			 'Descuentos' => [					
				 'queryBuilder' => function ($q) {
					 return $q->where(['tipo_oferta in ("RV","RR","OR","TD","RL","SS","FR","TH","VC")']); // Full conditions for filtering
				 }
				 //'tipo_oferta in ("RV","RR","OR","TD","RL","SS","FR","TH","TD")',
			 ...
			 ->hydrate(false)
				 ->join([
					 ...
					 'd.tipo_venta = "D"',
					 'd.fecha_hasta >=' => $fecha,
					 'd.tipo_oferta in ("RV","RR","OR","TD","RL","SS","FR","TH","VC")'
					 ]		
				 ]
				 )
				 ->join([
        ...
        foreach ($carritocon as $carrito) :
            ...
            if (($carrito['categoria_id'] != 5) && ($carrito['categoria_id'] != 4)  && ($carrito['categoria_id'] != 3) && ($carrito['categoria_id'] != 2)) {
                ...
                if ($carrito['descuento'] > 0) {
                    ...
                    $subtotal = $precio * $cant_carrito;
                    if (($this->request->session()->read('Auth.User.codigo_postal') == 9410 || $this->request->session()->read('Auth.User.codigo_postal') == 9420) && $carrito['articulo']['id'] == 16144) {
                        $subtotal = $subtotal * 1.224;
                    }
                }
                if ($carrito['articulo']['cadena_frio'] == 1 && $carrito['articulo']['subcategoria_id'] != 10)
