...
<div id="flotante"></div>
<?php $indice=0; $cat = $categorias; $titulolab=0; $preciot=""; $articulos2 = $articulos->toArray();?>
<?= $this->Form->create('CarritosPreventas',['url'=>['controller'=>'CarritosPreventas','action'=>'carritoaddall'],'id'=>'formaddcart','onsubmit'=>'return validaragregar()']); ?>
<div class ="col-md-x" >
...
if ($articulo['laboratorio_id']!= $titulolab || $first ==0 || $preciot != $articulo['preventas'][0]['tipo_precio'])
{
if ($first !=0)
...
echo '<tr><th>Cant.</th><th>'. $this->Paginator->sort('descripcion_pag','Descripción').'</th><th>';
/*if ($articulo['descuentos'] !=null)	if ($articulo['descuentos'][0]['tipo_precio']=='P')
echo 'P.S.P.';
else
echo 'P.F.';*/
...
echo $this->Form->input($encabezado.'descuento',['type'=>'hidden','value'=>$articulo['preventas'][0]['dto_drogueria']]); 	
...
<td class='masinfoband'>
<?php 
$cat2 = $articulo['categoria_id'];?>
<div onmouseover="showdiv(event,'<?php 
echo str_replace('"', '', $articulo['descripcion_pag']).'</br>'; 
echo 'Laboratorio: '.$articulo['laboratorio']['nombre'].'</br>';
echo 'Categoría: '.$cat2.'</br>';
echo 'Troquel: '.$articulo['troquel'].'</br>';
echo 'EAN: '.$articulo['codigo_barras'].'</br>';
?>','<?php echo $articulo['iva'];?>','<?php echo $articulo['trazable'];?>','<?php echo $articulo['cadena_frio'];?>'
,'<?php echo $articulo['categoria_id'];?>','<?php echo $articulo['pack'];?>','<?php echo $articulo['fv_cerca'];?>'
...
<td class="td-sub-tabla">
<?php echo ' <font color="red" style="font-weight: bold;">'.$articulo['preventas'][0]['dto_drogueria'].'% '.'</font>'; ?>
</td>
<td class="td-sub-tabla">
<?php echo $articulo['preventas'][0]['plazo'];?>
</td>
