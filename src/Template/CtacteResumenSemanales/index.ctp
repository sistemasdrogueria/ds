<div class="col-md-3">
    <div class="product-item-3"> 
		<div class="product-content3">	
			<h4><?php echo 'Resumen Semanal';?></h4>
			<script>
			...
			function pad(x) {
				...
				var nombre = "RESU"+pad2(xyz)+pad(x)+".pdf";
					$.ajax({
							data: {
							...
							$("#link_descarga_pdf").html('<a href="' + location + '/downloadfile/' + x + '/' + xyz + '"><img src="/ds3/img/pdf.png" title="Descargar PDF" alt="PDF"></a>');
							 $("#link_descarga_xls").html('<a href="'+location+'/downloadexcel/'+x+'/'+xyz+'"><img src="https://drogueriasur.com.ar/ds/img/excel.png" title="Descargar Excel" alt="Excel"></a>');
						...
						var text = getSelectedText('selectsemananro');
						$("#titulo_oferta").html(text);
						$("#link_descarga_pdf").html('<a href="'+location+'/downloadfile/'+x+'/'+xyz+'"><img src="https://drogueriasur.com.ar/ds/img/pdf.png" title="Descargar PDF" alt="PDF"></a>');
						$("#link_descarga_xls").html('<a href="'+location+'/downloadexcel/'+x+'/'+xyz+'"><img src="https://drogueriasur.com.ar/ds/img/excel.png" title="Descargar Excel" alt="Excel"></a>');
						$("#link_descarga_label").html("Descargar");*/
				
				//var nombre = "RESU"+pad2(xyz)+pad(x)+".pdf";
				//var nombre = "RESU"+"<?php echo str_pad($this->request->session()->read('Auth.User.codigo'), 6, "0", STR_PAD_LEFT);?>"+pad(x)+".pdf";
				/*		$("#cuerpo_oferta").html('<iframe src="http://docs.google.com/gview?url=http://drogueriasur.com.ar/ds/webroot/temp/Comprobantes/'+nombre+'&embedded=true" style="width:90%; height:400px;" frameborder="0"></iframe>');
						var text = getSelectedText('selectsemananro');
						$("#titulo_oferta").html(text);
						$("#link_oferta").html('<a href="http://www.drogueriasur.com.ar/ds/ctacte_resumen_semanales/downloadfile/'+x+'/'+xyz+'">Descargar Archivos Aqui</a>');*/
				}
				...
				<tr>
					<td>
						...
						//,'label'=>'Selecione la semana'
							...
							echo $this->Form->input('nro_sistema', ['id'=>'selectsemananro','options' => $ctacteResumenSemanales, 'onchange'=>'selectsemana(this);']);
						?>
						</div>
	...
	<div class="product-item-3"> 
		...
		<h3 id="titulo_oferta" align="center"></h3>
			...
			<div class="row" align="center" style="display:inline-block; text-align:center">
			  ...
			  <div id="link_descarga_xls"  style="float: left; margin:10px;">  </div>
			</div>
...
<h6>Bahia Blanca 
<?php echo ', '.date_format($novedade['fecha'],'d-m-Y'); ?></h6>
<h4 class="name">
<?= h($novedade->titulo) ?>
</h4>
...
<?php 
//if ($novedade->img_file =="")
echo nl2br(h($novedade->descripcion_completa)); 
//else
?>
</p>
</div><!-- /.member-details -->
...
if ($novedade['archivopdf']>0)
{
echo '<div><iframe src="https://docs.google.com/gview?url=https://drogueriasur.com.ar/ds/webroot/img/novedades/'.$novedade->img_file2.'&embedded=true" style="width:95%; min-height:550px;" frameborder="0"></iframe></div>';						
}	
}
?>
</div><!-- /.inner-container -->
