...
class TicketsController extends AppController
{
    ...
    {
		 if (in_array($this->request->action, ['edit_admin', 'delete_admin','add_admin','index_admin','index_admin_search','view_admin','enviar_ticket_mail','ticket_mail','sendReclamo','excel'])) {
			...
			{		
				if (in_array($this->request->action, ['view', 'add','index','search','searchitem','add_ticket','add_item','add_item_reclamo','confirm','add_comprobante_item','ticketpdf','resumen','recall'])) {
	 ...
	 public function ticketpdf($id = null)
    {
        ...
        //$this->viewBuilder()->layout('pdf/reclamo');
        $reclamo = $this->Reclamos->get($id, [
        ...
        if ($this->request->is('post','get')) {
			...
			$query = $this->FacturasCabeceras->find('all')	
					->contain(['FacturasCuerposItems','FacturasCuerposItems.Articulos' => [			
						'queryBuilder' => function ($q) {
							return $q->where(['FacturasCuerposItems.Articulos.eliminado =0']); // Full conditions for filtering
						}
					]]) 
					->hydrate(false)
			...
			else
			{
				...
				$query = $this->Pedidos->find('all')	
					->contain(['PedidosItems','PedidosItems.Articulos'])//,'Articulos'
					->hydrate(false)
            ...
            //$reclamo = $this->Reclamos->patchEntity($reclamo, $this->request->data);
 			if  ( $this->request->session()->read('reclamo')!=null)		
				...
				if ($reclamo_items!=null)
				{
					...
					return $this->redirect(['controller'=>'Tickets','action' => 'resumen',$id['id'] ]);
					} else {
