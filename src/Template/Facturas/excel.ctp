...
$objPHPExcel->getProperties()->setTitle("Listado de facturas"); //titulo
...
$fila+=1;
		...
		$objPHPExcel->getActiveSheet()->SetCellValue("C$fila", 'N PEDIDO');
		$objPHPExcel->getActiveSheet()->SetCellValue("D$fila", 'CLIENTE');
		$objPHPExcel->getActiveSheet()->SetCellValue("E$fila", 'LETRA COMP.');
		$objPHPExcel->getActiveSheet()->SetCellValue("F$fila", 'NÚMERO COMP.');
...
$objPHPExcel->getActiveSheet()->getStyle($filas)->getFont()->setBold(true); //negrita
  ...
  foreach ($facturasCabeceras as $facturasCabecera):
		...
		$objPHPExcel->getActiveSheet()->SetCellValue("D$fila", $clientes[$this->request->session()->read('cliente_id')]);		
...
$namefile= '"Facturas_'.$clientes[$this->request->session()->read('cliente_id')].'.xlsx"';
//// nombre del archivo
