...
.col-item {padding: 0;border: 5px solid #fff;background: 0 0;margin-bottom: 50px;}
.col-item .photo {position: relative;	max-width : 150px;}
...
<div id=slider_container_fragancia style="width:100%">
...
foreach ($incorporations as $incorporation): 
		...
		echo '<div>'.$this->Html->image('incorporations/selectivas/'.$incorporation['imagen'],['data-u'=>'image']).'</div>';
endforeach; ?>
</div>
...
function ScaleSlider() {
var slider_container_fragancia = document.getElementById('slider_container_fragancia');
var bodyWidth = slider_container_fragancia.clientWidth;
