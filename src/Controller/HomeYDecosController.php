...
*/
	...
	public function index()
	{
		...
		$marcas = $this->Marcas->find('list', ['keyField' => 'id','valueField' => 'nombre'])->where(['marcas_tipos_id'=>17])->order(['nombre' => 'ASC']);
		$marcas->toArray();
		$this->set(compact('marcas'));
		...
		$publicationzocalon = $this->Publications->find('all')->where(['fecha_hasta >=CURRENT_DATE()', 'habilitada' => '1', 'ubicacion' => '23'])->order(['orden' => 'ASC'])->all();
	...
	public function search($marcaid =null, $grupoid =null)
	{
		...
		$articulosA = $this->Articulos->find('all')
			...
			$articulosA->where(['Articulos.subcategoria_id'=>25]);

		if ($termsearch2!="") {
			$articulosA->andWhere([
				'OR' => [
					['Articulos.descripcion_pag LIKE' => $termsearch2],
					['Articulos.descripcion_sist LIKE' => $termsearch2],
					['Articulos.troquel LIKE' => $termsearch2],
					['Articulos.codigo_barras LIKE' => $termsearch2],
					['Articulos.codigo_barras2 LIKE' => $termsearch2]
				],
			]);	
		}
		if ($marcaid !=0)
		...
		if ($grupoid ==0 && $marcaid ==0 && $termsearch2 =="")
		$articulosA->where(['Articulos.marca_id in (select id from marcas where marcas_tipos_id=17)']);
		...
		$this->loadModel('Publications');
			...
			->where([
				...
				'Articulos.stock <>' => 'D',
				'Marcas.id IS NOT' => null // Excluir marcas nulas
			])
			->group(['Marcas.id'])
			->order(['Marcas.nombre' => 'ASC'])
			->select(['Marcas.id','Marcas.nombre','Marcas.imagen','Articulos.grupo_id']); 
				
			$marcas = $articulosMarcasGrupo->map(function ($row) {
				return [
					'id' => $row['marca']['id'],
					'nombre' => $row['marca']['nombre'],
				];
			})->combine('id', 'nombre')->toArray();
			}
			else
			{
				if ($grupoid ==0 && $marcaid ==0 && $termsearch2 =="")
