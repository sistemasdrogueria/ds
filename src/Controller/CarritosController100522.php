...
class CarritosController extends AppController
{
	...
	public function search_bf($terms_marca = null, $terms_lab=null, $terms=null,$terms_mult = null)
	{
		...
		if 	($this->request->session()->read('Auth.User.farmapoint')>0)
			{
				...
				$list_tipo_dto = '("RV","RR","OR","TD","RL","SS","FR","TH","PS","FP","SC")';
			}
		else
			{
				$list_tipo_dto_dist = '[tipo_oferta <> "FP" , tipo_oferta <> "VC"]';
				$list_tipo_dto = '("RV","RR","OR","TD","RL","SS","FR","TH","PS","SC")';
		  ...
		  $articulosA = $this->Articulos->find()
				...
				->hydrate(false)
					->join([
						...
						'd.tipo_oferta in '.$list_tipo_dto//("RV","RR","OR","TD","RL","SS","FR","TH","PS","FP")'
						]		
					]
					);
		...
		if ($terms!="" && $terms_m =="")
		{
			$articulosA->andWhere([
					...
					'OR' => [['Articulos.descripcion_pag LIKE'=>$terms], 
	...
	if ($lab_id ==387)
			{
				$articulos->where(['Articulos.clave_amp in (2675	,
				10688	,
				28632	,
				23439	,
				23559	,
...
/*
	...
	public function index()
    {
		...
		$ofertasX = $this->Ofertas->find('all')->where(['oferta_tipo_id in (2,4,5,6,7,11,19)','activo'=>1,'fecha_desde <= CURRENT_DATE()','fecha_hasta >=CURRENT_DATE()'])->order(['oferta_tipo_id'=>'ASC','orden' => 'ASC']);
		...
		*/
        if (36016 != $this->request->session()->read('Auth.User.cliente_id') )
		{
	...
	public function dulzura( $termsearchlab = null,$termsearch = null)
	{
		...
		$articulosA = $this->Articulos->find()
			->contain([
				'Descuentos' => [
					'queryBuilder' => function ($q) {
						return $q->where(['tipo_oferta <> "FP"', 'tipo_oferta in ("RV","RR","OR","TD","RL","SS","FR","TH")', 'tipo_oferta <> "VC"']); // Full conditions for filtering
					}
			...
			->join(
				[
					...
					'conditions' => [
						...
						'd.tipo_venta = "D"',
						'd.fecha_hasta >=' => $fecha->i18nFormat('yyyy-MM-dd'),
						'd.tipo_oferta in ("RV","RR","OR","TD","RL","SS","FR","TH")'
					]
				]
			);
		//->where(['eliminado'=>0]);
 ...
 /*
	...
	public function confirm()
    {
		...
		if ((36932 != $this->request->session()->read('Auth.User.cliente_id')) && (36241 != $this->request->session()->read('Auth.User.cliente_id') ))
		...
		//$fecha->i18nFormat('YYYY-MM-dd');
	  	$articulosA = $this->Articulos->find()
				...
				->hydrate(false)
					...
					->where(['Articulos.laboratorio_id in (462,382,503)']);
	...
	public function view()
    {
		...
		//$fecha = Time::createFromFormat('d/m/Y',$fecha,'America/Argentina/Buenos_Aires');
		 ...
		 if ($this->request->session()->read('Auth.User.farmapoint'))
		 {
			 ...
			 'Descuentos' => [					
				 'queryBuilder' => function ($q) {
					 return $q->where(['tipo_oferta in ("RV","RR","OR","TD","RL","SS","FR","TH","FP","VC","CM","FF","BF","SC")']); // Full conditions for filtering
				 }
				 //'tipo_oferta in ("RV","RR","OR","TD","RL","SS","FR","TH","TD")',
			 ...
			 ->hydrate(false)
				 ->join([
					 ...
					 'd.tipo_venta = "D"',
					 'd.fecha_hasta >=' => $fecha,
					 'd.tipo_oferta in ("RV","RR","OR","TD","RL","SS","FR","TH","FP","VC","CM","FF","BF","SC")'
					 ]		
				 ]
				 )
				 ->join([
				 ...
				 )->where(['c.cliente_id' => $this->request->session()->read('Auth.User.cliente_id'), 'c.cantidad  < c.unidad_minima', 'c.unidad_minima IS NOT NULL'])->order(['Articulos.descripcion_sist'=> 'ASC']);
		 ...
		 else
		 {
			 ...
			 'Descuentos' => [					
				 'queryBuilder' => function ($q) {
					 return $q->where(['tipo_oferta in ("RV","RR","OR","TD","RL","SS","FR","TH","VC","CM","FF","BF","SC")']); // Full conditions for filtering
				 }
				 //'tipo_oferta in ("RV","RR","OR","TD","RL","SS","FR","TH","TD")',
			 ...
			 ->hydrate(false)
				 ->join([
					 ...
					 'd.tipo_venta = "D"',
					 'd.fecha_hasta >=' => $fecha,
					 'd.tipo_oferta in ("RV","RR","OR","TD","RL","SS","FR","TH","VC","CM","FF","BF","SC")'
					 ]		
				 ]
				 )
				 ->join([
	...
	*/
	  	...
	  	$articulo = $this->Articulos->find()
				...
				->hydrate(false)
					->join([
						...
						'd.tipo_venta = "D"',
						'd.fecha_hasta >=' => $fecha,
						'd.tipo_oferta in ("RV","RR","OR","TD","RL","SS","VC","FR","PS")'
						]		
					]
					)
					->where([
	...
	public function hotsale_search($termsearch = null, $termsearchlab=null)
    {
		...
		//$fecha->i18nFormat('YYYY-MM-dd');
	  	$articulosA = $this->Articulos->find()
					...
					'Descuentos' => [					
						'queryBuilder' => function ($q) {
							return $q->where(['tipo_oferta <> "FP"','tipo_oferta in ("RV","RR","OR","TD","RL","SS","FR,"PS")','tipo_oferta <> "VC"']); // Full conditions for filtering
						}
				...
				->hydrate(false)
					->join([
						...
						'd.tipo_venta = "D"',
						'd.fecha_hasta >=' => $fecha->i18nFormat('yyyy-MM-dd'),
						'd.tipo_oferta in ("RV","RR","OR","TD","RL","SS","FR","PS")'
						]		
					]
					);
				//->where(['eliminado'=>0]);
	...
	public function search($termsearch = null, $termsearchlab=null)
    {
		...
		->hydrate(false)
			->join([
				...
				'd.tipo_venta = "D"',
				'd.fecha_hasta >=' => $fecha->i18nFormat('yyyy-MM-dd'),
				'd.tipo_oferta in ("RV","RR","OR","TD","RL","SS","FR","TH","PS","FP")'
				]		
			]
			);
	...
	public function search_i($terms = null, $terms_lab=null,$terms_mult = null)
    {
		...
		if 	($this->request->session()->read('Auth.User.farmapoint')>0)
			{
				$list_tipo_dto_dist = 'tipo_oferta <> "VC"';
				$list_tipo_dto = '("RV","RR","OR","TD","RL","SS","FR","TH","PS","FP")';
			}
		else
			{
				$list_tipo_dto_dist = '[tipo_oferta <> "FP" , tipo_oferta <> "VC"]';
				$list_tipo_dto = '("RV","RR","OR","TD","RL","SS","FR","TH","PS")';
	...
	public function search_cm($terms = null, $terms_lab=null,$terms_mult = null)
    {
		...
		if 	($this->request->session()->read('Auth.User.farmapoint')>0)
			{
				$list_tipo_dto_dist = 'tipo_oferta <> "VC"';
				$list_tipo_dto = '("OR","TD","RL","SS","FR","TH","PS","FP","CM","SC")';
			}
		else
			{
				$list_tipo_dto_dist = '[tipo_oferta <> "FP" , tipo_oferta <> "VC"]';
				$list_tipo_dto = '("OR","TD","RL","SS","FR","TH","PS","CM","SC")';
	  	...
	  	$articulosA = $this->Articulos->find('all')
				...
				->hydrate(false)
					->join([
						...
						'd.tipo_venta = "D"',
						'd.fecha_hasta >=' => $fecha->i18nFormat('yyyy-MM-dd'),
						'd.tipo_oferta in ("RV","RR","OR","TD","RL","SS","VC","PS")'
						]		
					]
				 );
	...
	/*
		...
		if ((36932 != $this->request->session()->read('Auth.User.cliente_id')) && (36241 != $this->request->session()->read('Auth.User.cliente_id')) && (35473 !=  $this->request->session()->read('Auth.User.cliente_id') ))
	...
	public function importconfirm ()
	{
		...
		$confirmados = $connection->execute('INSERT INTO carritos (id, cliente_id, 	articulo_id, 	descripcion, 	cantidad, 	precio_publico, 	descuento, 	unidad_minima, 	tipo_precio, 	plazoley_dcto,	combo_id, 	tipo_oferta, 	tipo_oferta_elegida, 	tipo_fact, creado, 	modificado, categoria_id, compra_max, descuento_id) 
			SELECT  NULL, cliente_id, 	articulo_id, 	descripcion, 	cantidad, 	precio_publico, 	descuento, 	unidad_minima, 	tipo_precio, 	plazoley_dcto,	combo_id, 	tipo_oferta, 	tipo_oferta_elegida, 	tipo_fact, creado, 	modificado ,categoria_id, compra_max ,descuento_id
	...
	public function vel()
    {
		...
		$ofertas = $connection->execute('SELECT ofertas.*, articulos.precio_publico, 
		  descuentos.dto_drogueria, descuentos.articulo_id, descuentos.plazo, descuentos.uni_min, descuentos.tipo_oferta, descuentos.tipo_venta, descuentos.tipo_precio 
		  FROM ofertas LEFT JOIN articulos on articulos.id = ofertas.articulo_id LEFT JOIN descuentos on descuentos.articulo_id =  articulos.id and 
		  descuentos.tipo_venta = "D" and	descuentos.fecha_hasta >=CURRENT_DATE() and	descuentos.tipo_oferta in ("RV","RR","OR","TD","RL","SS","VC","PS") where ofertas.activo=1')->fetchAll('assoc');
		$this->set('ofertas',$ofertas);
			...
			if (((int)$quantity > 0) && ((int)$quantity < 10000)) {
				...
				if($validar){
					if ($this->Carritos->save($carro)) {
						...
						$responseData = ['success' => true, 'responseText' => "ok", 'status' => 200, 'carros' => $carro, 'contenidocarro' => $contenidocarrito, 'subtotal' => $subtotales];
					} else {
				...
				if ((int)$quantity == 0) {
					...
					if ($carritos != null) {
						...
						$contenidocarrito = $this->contenidoCarrito();
						$subtotales = $this->calcularsubtotales($contenidocarrito,$carro['articulo_id'] );
						$responseData = ['success' => true, 'responseText' => "eliminado", 'status' => 200, 'carros' => $carritos, 'contenidocarro' => $contenidocarrito, 'subtotal' => $subtotales];
			...
			if (((int)$quantity > 0) && ((int)$quantity < 10000)) {
				...
				if($validar){
					if ($this->CarritosTemps->save($carro)) {
						...
						$responseData = ['success' => true, 'responseText' => "ok", 'status' => 200, 'subtotal' => $subtotales,'contenidocarrotemps' => $contenidocarritotemps];
					} else {
						$responseData = ['success' => false, 'responseText' => "'No se pudo modificar la cantidad correctamente,'", 'status' => 400];
						$this->Flash->error('No se pudo agregar al carro de compras. Intente de nuevo', ['key' => 'changepass']);
					}
				...
				if ((int)$quantity == 0) {
					...
					if ($carritos != null) {
						...
						if ($this->CarritosTemps->delete($carritos))
							...
							$contenidocarritotemps = $this->contenidoCarritoTemps();
							$subtotales = $this->calcularsubtotales($contenidocarritotemps);
						    $responseData = ['success' => true, 'responseText' => "eliminado", 'status' => 200, 'carros' => $carritos, 'contenidocarrotemps' => $contenidocarritotemps, 'subtotal' => $subtotales];
	...
	public function importresulttemp()
	{
		...
		$this->sumacarrito();
		$this->sumacarritotemp();
	//$this->request->data['filetext']="";
	//$this->request->session()->write('listaarray',null);
	//$this->request->session()->write('tablaped',null);
	}
