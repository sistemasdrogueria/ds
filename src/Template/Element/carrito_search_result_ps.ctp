...
if ($articulo['descuentos'][1]['tipo_venta']=='D')
	{
	$articulo_descuento = $articulo['descuentos'][0];

	if ($articulo['descuentos'][0]['dto_drogueria'] >$articulo['descuentos'][1]['dto_drogueria'])
		{
			$articulo_descuento = $articulo['descuentos'][0];
		}
		else
		$articulo_descuento = $articulo['descuentos'][1];
	}
	else
	$articulo_descuento = $articulo['descuentos'][0];
}
...
if ($articulo_descuento['tipo_venta']=='D')
{

echo $this->Form->input($encabezado.'plazoley_dcto',['type'=>'hidden','value'=>$articulo_descuento['plazo']]); 	
echo $this->Form->input($encabezado.'unidad_minima',['type'=>'hidden','value'=>$articulo_descuento['uni_min']]); 	
echo $this->Form->input($encabezado.'tipo_oferta',['type'=>'hidden','value'=>$articulo_descuento['tipo_oferta']]); 
echo $this->Form->input($encabezado.'tipo_venta',['type'=>'hidden','value'=>$articulo_descuento['tipo_venta']]); 
echo $this->Form->input($encabezado.'tipo_precio',['type'=>'hidden','value'=>$articulo_descuento['tipo_precio']]); 
echo $this->Form->input($encabezado.'combo_id',['type'=>'hidden','value'=>$articulo_descuento['combo_id']]); 
echo $this->Form->input($encabezado.'multiplo',['type'=>'hidden','value'=>$articulo_descuento['multiplo']]); 
echo $this->Form->input($encabezado.'combo_tipo_id',['type'=>'hidden','value'=>$articulo_descuento['combo_tipo_id']]);
echo $this->Form->input($encabezado.'descuento_id',['type'=>'hidden','value'=>$articulo_descuento['id']]); 

if ($articulo_descuento['tipo_oferta']!='TH')
	{
		echo $this->Form->input($encabezado.'descuento',['type'=>'hidden','value'=>$articulo_descuento['dto_drogueria']]); 	
	}
	else
	{
		$descuento_off=$articulo_descuento['dto_drogueria']+$condiciongeneral;
...
if ($articulo_descuento['tipo_venta']=='D')
{
	if ($articulo_descuento['tipo_oferta']!='TH')
	{
		$texto= $texto .'Oferta: '.$articulo_descuento['dto_drogueria'].'% por '.$articulo_descuento['uni_min'].'unidad(es)</br>';
	}
	else
	{
		$descuento_off = number_format(round($articulo_descuento['dto_drogueria']+$condiciongeneral, 3),2,',','.') ;
		$texto= $texto .'Oferta: '.$descuento_off.'% por '.$articulo_descuento['uni_min'].'unidad(es)</br>';
	}
	$texto= $texto .'Plazo: '. $articulo_descuento['plazo'].'</br>';
	if ($articulo_descuento['tipo_oferta']!='TH')
	{
		$texto= $texto .'Tipo de oferta: '. $articulo_descuento['tipo_oferta'].'</br>';
	}
	else
	{
...
if ($articulo_descuento['tipo_venta']=='D' and $articulo['stock']!='F')
{
...
{
if ($articulo_descuento['tipo_venta']=='D' )	
//echo ' <font color="red" style="font-weight: bold;">'.$articulo_descuento['dto_drogueria'].'% '.'</font>'; 
if ($articulo_descuento['tipo_precio']=='P')
	{
		if ($articulo_descuento['tipo_oferta']!='TH')
			echo ' <font color="red" style="font-weight: bold;">'.$articulo_descuento['dto_drogueria'].'% '.'</font>'; 
		else {
			echo ' <font color="red" style="font-weight: bold;">'.number_format(round($articulo_descuento['dto_drogueria']+$condiciongeneral, 3),2,',','.').'% '.'</font>'; 
	...
	{
		if ($articulo['iva']==0)
		{$condiciongeneralfinal = 100*(1-($descuento_pf * (1-$condicion/100)*(1-$articulo_descuento['dto_drogueria']/100)));
...
if ($articulo['descuentos'] !=null and $articulo['stock']!='F'){
if ($articulo_descuento['tipo_venta']=='D')
echo $articulo_descuento['uni_min'];
else
if (count($articulo['descuentos'])>1)
if ($articulo['descuentos'][1]['tipo_venta']=='D')
...
if ($articulo_descuento['tipo_venta']=='D')
{
	if ($articulo_descuento['tipo_oferta']!='TH')
	{
		echo $articulo_descuento['tipo_oferta'];
	}
	else
	{
		echo 'TD';
